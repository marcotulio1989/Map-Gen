...
class IslandGeneratorApp {
  ...
  private foliageAreaMesh: THREE.Mesh | null = null;
  ...
  private async generateIsland() {
    ...
    try {
        ...
        if (this.foliageAreaMesh) {
            this.scene.remove(this.foliageAreaMesh);
            this.foliageAreaMesh.geometry.dispose();
            (this.foliageAreaMesh.material as THREE.Material).dispose();
            this.foliageAreaMesh = null;
        }
        ...
        if (topContourVertices.length > 2) {
            ...
            // --- Generate Foliage Area Visualization ---
            const numContourPoints = topContourVertices.length;
            if (numContourPoints > 2 && innerRefPoints.length === numContourPoints) {
                const vizVertices: number[] = [];
                const vizIndices: number[] = [];
                const yOffset = 0.02; // To prevent z-fighting

                topContourVertices.forEach(p => vizVertices.push(p.x, p.y + yOffset, p.z));
                innerRefPoints.forEach(p => vizVertices.push(p.x, p.y + yOffset, p.z));
                ...
                for (let i = 0; i < numContourPoints; i++) {
                    ...
                    vizIndices.push(outer_curr, inner_curr, outer_next);
                    vizIndices.push(inner_curr, inner_next, outer_next);
                }

                const foliageAreaGeometry = new THREE.BufferGeometry();
                foliageAreaGeometry.setAttribute('position', new THREE.Float32BufferAttribute(vizVertices, 3));
                foliageAreaGeometry.setIndex(vizIndices);

                const foliageAreaMaterial = new THREE.MeshBasicMaterial({
                ...
                this.foliageAreaMesh = new THREE.Mesh(foliageAreaGeometry, foliageAreaMaterial);
                this.scene.add(this.foliageAreaMesh);
            }
  ...
  private generateFoliage(outerContour: THREE.Vector3[], innerContour: THREE.Vector3[], foliageBandWidth: number) {
    ...
    setTimeout(() => {
        try {
            ...
            const numInstances = Math.floor(totalArea / 100 * grassDensity);
